...
import { useUpdateWebsiteViews } from './use-update-website-views';
...
describe('useUpdateWebsiteViews request', () => {
	...
	it('invalidate query get website views when is success', async () => {
		...
		MockAxios.onPatch(requestUrl).reply(201, response);
		const { result } = renderHook(useUpdateWebsiteViews, { wrapper });
	...
	it('no invalidate query get website views when is error', async () => {
		const SpyInvalidateQueries = jest.spyOn(queryClient, 'invalidateQueries');
		MockAxios.onPatch(requestUrl).reply(401);
		const { result } = renderHook(useUpdateWebsiteViews, { wrapper });
